...
% E-mail addresses: yao.s.wang@gmail.com, andrew.pengjj@gmail.com
% -------------------------------------------------------------------------
...
d               = sizeD(3);
%% 
Eny_x   = weight(1)^2*( abs(psf2otf([+1; -1], [h,w,d])) ).^2  ;
Eny_y   = weight(2)^2*( abs(psf2otf([+1, -1], [h,w,d])) ).^2  ;
Eny_z   = weight(3)^2*( abs(psf2otf([+1, -1], [w,d,h])) ).^2  ;
...
for iter = 1: maxIter
    ...
    %% - update Z
    diffT_p  = diffT3_weight( beta*F - Gamma, sizeD,weight );
